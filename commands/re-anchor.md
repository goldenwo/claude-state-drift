...
Invoke the Skill tool with `claude-state-drift:re-anchor` against the current project. Do not inline or paraphrase the procedure — defer to the skill for the full contract (objective re-read → recent-work audit → alignment categories on-track/mild/significant → handoff to `update-state` if needed).
